...
export function AppShell({ children, caseId }: AppShellProps) {
  ...
  const [showLegalUpdates, setShowLegalUpdates] = useState(true);
      ...
      {caseId && (
        associateOpen ? (
          <aside className="hidden lg:flex w-[360px] xl:w-[380px] bg-[#111111] border-l border-border flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-30">
            ...
            {/* Persistent Legal Intelligence Banner */}
            {legalUpdates.length > 0 && (
              <div className="px-3 pt-2 pb-1 bg-[#14121e] border-b border-white/5 flex-shrink-0">
                <div className="flex items-center justify-between pb-1 text-[11px] text-muted-foreground">
                  <div className="flex items-center gap-1.5 font-medium text-[#C084FC]">
                    <Scale className="w-3.5 h-3.5 text-[#A855F7]" />
                    <span>Legal Intelligence ({legalUpdates.length})</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setShowLegalUpdates(prev => !prev)}
                    className="text-[10px] text-muted-foreground hover:text-white hover:underline cursor-pointer"
                  >
                    {showLegalUpdates ? 'Minimize' : 'Expand'}
                  </button>
                </div>
                {showLegalUpdates && (
                  <RotatingLegalUpdate
                    updates={legalUpdates}
                    onInquire={(item) => setChatInput(`How does ${item.title} apply to our matter?`)}
                    className="my-1"
                  />
                )}
              </div>
                    ...
                    `}>
                      {msg.role === 'user' ? (
                        ...
                        <>
                          ...
                          {/* Live processing status while waiting */}
                          ...
                          {msg.streaming && legalUpdates.length > 0 && !showLegalUpdates && (
                            <RotatingLegalUpdate
                              updates={legalUpdates}
                              onInquire={(item) => setChatInput(`How does ${item.title} apply to our matter?`)}
                            />
